...
from tkinter import Image, messagebox as ms, filedialog, simpledialog
...
def mostrar_sugerencias():
    if not os.path.exists(REPO_ACTIVITY_FILE):
        return

    with open(REPO_ACTIVITY_FILE, "r", encoding="utf-8") as f:
        actividad = json.load(f)

    sugeridos = [repo for repo, data in actividad.items() if data["times_opened"] >= 5]
    if sugeridos:
        ms.showinfo("💡 Sugerencia", f"Has accedido frecuentemente a: {', '.join(sugeridos)}. ¿Deseas fijarlos?")

def registrar_actividad(repo_name):
    actividad = {}
    if os.path.exists(REPO_ACTIVITY_FILE):
        try:
            with open(REPO_ACTIVITY_FILE, "r", encoding="utf-8") as f:
                actividad = json.load(f)
        except json.JSONDecodeError:
    ...
    if repo_name in actividad:
        actividad[repo_name]["times_opened"] += 1
        actividad[repo_name]["last_opened"] = now
    else:
        actividad[repo_name] = {"times_opened": 1, "last_opened": now}

    with open(REPO_ACTIVITY_FILE, "w", encoding="utf-8") as f:
...
def mostrar_terminal():
    ...
    terminal.configure(bg=TERMINAL_STYLE["bg"])

    term_output = tk.Text(terminal, bg=TERMINAL_STYLE["bg"], fg=TERMINAL_STYLE["fg"],
                          insertbackground="white", font=TERMINAL_STYLE["font"])
    ...
    def ejecutar_comando(event=None):
        ...
        historial_index = len(historial)

        imprimir_salida(f"{comando}", "stdout")
        manejar_comando(comando)

    def manejar_comando(comando):
        args = comando.strip().split()
        if not args:
            return
        ...
        if cmd == "help":
            ...
            imprimir_salida("🔍 Searching available versions...")
    ...
    def navegar_historial(event):
        ...
        if event.keysym == "Up":
            ...
            historial_index = min(len(historial), historial_index + 1)

        if 0 <= historial_index < len(historial):
            entrada.delete(0, 'end')
            entrada.insert(0, historial[historial_index])
        else:
            entrada.delete(0, 'end')

    def limpiar_terminal(event=None):
        term_output.config(state='normal')
        term_output.delete("1.0", "end")
        term_output.insert("end", "➜ ", "stdout")
        term_output.config(state='disabled')

    def mostrar_ayuda(event=None):
        manejar_comando("help")

    entrada.bind("<Return>", ejecutar_comando)
    entrada.bind("<Up>", navegar_historial)
    entrada.bind("<Down>", navegar_historial)
    entrada.bind("<Control-l>", limpiar_terminal)
    entrada.bind("<F1>", mostrar_ayuda)
    entrada.focus()

    imprimir_salida("# Management terminal started. Press F1 for help..")
    terminal.mainloop()
    
def show_plugins_window():
    plugins_window = tk.Toplevel(root)
    plugins_window.title("Plugins")
    plugins_window.iconbitmap(path_icon)
    
    ttk.Label(plugins_window, text="Plugins", font=("Arial", 14, "bold")).pack(pady=5)
    
    plugins_list = ttk.Frame(plugins_window)
    plugins_list.pack(expand=True, fill="both", padx=5, pady=5)
    
    list_plugins = tk.Listbox(plugins_list, height=10, width=50)
    list_plugins.pack(expand=True, fill="both", padx=5, pady=5, side="left")
    
    scrollbar = ttk.Scrollbar(plugins_list, orient="vertical", command=list_plugins.yview)
    scrollbar.pack(side="left", fill="y")
    
    list_plugins.configure(yscrollcommand=scrollbar.set)
    
    plugins = {}
    for plugin_folder in os.listdir(PLUGINS_DIR):
        ruta_plugin = os.path.join(PLUGINS_DIR, plugin_folder)
        if os.path.isdir(ruta_plugin):
            for archivo in os.listdir(ruta_plugin):
                if archivo.endswith(".exe"):
                    plugins[plugin_folder] = os.path.join(ruta_plugin, archivo)
                    list_plugins.insert("end", plugin_folder)
                    break
            
    def execute_plugin(event=None):
        selected = list_plugins.curselection()
        if not selected:
            return
        
        name = list_plugins.get(selected[0])
        path = plugins.get(name)
        try:
            subprocess.Popen([path], shell=True)
        except Exception as e:
            ms.showerror("Error", f"Error executing plugin: {e}")
...
root.geometry("1265x550")
root.iconbitmap(path_icon)
root.resizable(True, True)
modo_confidencial = tk.BooleanVar(value=False)
TERMINAL_SHELL = tk.StringVar(value="internal")
root.bind("<Control-Shift-L>", lambda e: mostrar_terminal())
...
def mostrar_selector_cuenta(master_frame):
    ...
    def agregar_cuenta():
        ...
        if token and is_github_token_valid(token):
            ...
            ms.showerror("❌ Error", "Invalid or not entered token.")
...
def actualizar_username_label():
    if username_label:
        username_display = "***" if modo_confidencial.get() else cuenta_seleccionada["username"]
        username_label.config(text=f"Username: {username_display}")

def iniciar_aplicacion():
    global GITHUB_TOKEN, GITHUB_USER, username_label
    GITHUB_TOKEN = cuenta_seleccionada["token"]
    GITHUB_USER = cuenta_seleccionada["username"] or obtener_usuario()
    
    if cuenta_seleccionada.get("avatar_url"):
        try:
            response = requests.get(cuenta_seleccionada["avatar_url"])
            img_data = response.content
            img = Image.open(BytesIO(img_data)).resize((32, 32))
            avatar = tk.PhotoImage(img)
            avatar_label = ttk.Label(root, image=avatar)
            avatar_label.image = avatar
            avatar_label.pack(side='top', padx=5)
        except:
            pass
    
    username_label = ttk.Label(root)
    username_label.pack(side='right')
    actualizar_username_label()
    ttk.Checkbutton(root, text="🔐 Confidential", variable=modo_confidencial, command=actualizar_username_label).pack(side="right")
    
    scopes = obtener_scopes(GITHUB_TOKEN)
    if scopes:
        scopes_label = ttk.Label(root, text=f"Permissions: {', '.join(scopes)}", font=("Segoe UI", 8), foreground="Yellow")
        scopes_label.pack(side='left', padx=5)
